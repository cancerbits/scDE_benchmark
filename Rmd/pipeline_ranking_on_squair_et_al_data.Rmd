---
title: 'Pipeline ranking on Squair et al. data with bulk as truth'
...
RES_DIR <- file.path(config$out_root, 'results', 'squair_et_al_data_summarized')
...
sum_res <- readRDS(file = file.path(RES_DIR, sprintf('performance_%s.Rds', ds_name)))
```

Make comparison names shorter
```{r}
sum_res <- mutate(sum_res,
                 comparison = stringr::str_replace_all(string = comparison, pattern = 'mouse', replacement = 'rep'),
                 comparison = stringr::str_replace_all(string = comparison, pattern = '_', replacement = '_vs_'))
```
...
Rescale MCC per comparison; average the comparisons
...
saveRDS(list(fr = fr, metrics_wide = metrics_wide), 
        file = file.path(config$out_root, 'results', sprintf('final_ranking_squair_et_al_%s.Rds', ds_name)))
select(fr, pos, transformation, de_method, MCC_rescaled, FDR_low, FDR_high, score, time) %>%
  readr::write_csv(file = file.path(config$out_root, 'results', sprintf('final_ranking_squair_et_al_data_experiments_%s.csv', ds_name)))
```
...
ch <- res_heatmap(fr, metrics_wide, 
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_big_%s.pdf', ds_name)), 
                  fig_width = 8, fig_height = 48)
```

```{r}
ch <- res_heatmap(fr, metrics_wide, 
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_big_squished_%s.pdf', ds_name)), 
...
ch <- res_heatmap(fr, metrics_wide, idx = which(pb),
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_big_pb_%s.pdf', ds_name)), 
                  fig_width = 8, fig_height = 16)
ch <- res_heatmap(fr, metrics_wide, idx = which(!pb),
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_big_sc_%s.pdf', ds_name)), 
                  fig_width = 8, fig_height = 26)
```
...
ch <- res_heatmap(fr, metrics_wide, idx = idx,
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_filter1_%s.pdf', ds_name)), 
...
ch <- res_heatmap(fr, metrics_wide, idx = idx,
                  filename = file.path(FIG_DIR, sprintf('ranking_squair_et_al_data_filter2_%s.pdf', ds_name)), 
