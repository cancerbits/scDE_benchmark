---
title: 'Combine pipeline rankings (real data)'
...
```{r}
DET_RES_DIR <- file.path(config$out_root, 'results', 'squair_et_al_data_summarized')

exclude <- 'CanoGamez2020_Th0-Th2|Angelidis2019_alvmac|Angelidis2019_pneumo|immune'

fpaths <- list.files(path = DET_RES_DIR, pattern = '^performance_', full.names = TRUE)
fpaths <- fpaths[!grepl(exclude, fpaths)]

sum_res <- lapply(fpaths, function(f) {
  ds <- stringr::str_remove(basename(f), pattern = 'performance_')
  ds <- stringr::str_remove(ds, '\\.Rds')
  readRDS(f) %>%
    mutate(dataset = ds) 
}) %>% bind_rows() 

sum_res <- filter(sum_res, de_method != 'scdeseq')
```

Average the replicates per dataset

```{r}
sum_res_avg <- group_by(sum_res, transformation, de_method, pipeline, expr_grp, dataset) %>%
  summarise(FDR = mean(FDR),
            Precision = mean(Precision),
            Sensitivity = mean(Sensitivity),
            Specificity = mean(Specificity),
            MCC = mean(MCC),
            time = mean(time), .groups = 'drop')
```

For every pipeline, get the MCC_rescaled, FDR_low, FDR_high, time

```{r}
# squish and rescale the MCC per experiment

fr <- filter(sum_res_avg, is.na(expr_grp)) %>%
  group_by(dataset) %>%
...
fdr_low <- filter(sum_res_avg, expr_grp == 'low') %>%
  group_by(transformation, de_method, pipeline) %>%
  summarise(FDR = mean(FDR), .groups = 'drop') %>%
  dplyr::rename(FDR_low = FDR)

fdr_high <- filter(sum_res_avg, expr_grp == 'high') %>%
  group_by(transformation, de_method, pipeline) %>%
  summarise(FDR = mean(FDR), .groups = 'drop') %>%
...
```{r}
metrics_wide <- filter(sum_res_avg, is.na(expr_grp)) %>%
                  arrange(dataset) %>%
                  tidyr::pivot_wider(
                              id_cols = c(pipeline, transformation, de_method),
                              names_from = c(dataset, dataset), 
...
saveRDS(list(fr = fr, metrics_wide = metrics_wide), 
        file = file.path(config$out_root, 'results', 'final_ranking_squair_et_al_combined.Rds'))
select(fr, pos, transformation, de_method, MCC_rescaled, FDR_low, FDR_high, score, time) %>%
  readr::write_csv(file = file.path(config$out_root, 'results', 'final_ranking_squair_et_al_combined.csv'))
```
...
res_heatmap <- function(fr, metrics_wide, idx = 1:nrow(fr), do_rescale = TRUE,
  ...
  ch <- Heatmap(mat_rescaled, cluster_columns = FALSE, cluster_rows = FALSE,
          ...
          column_names_gp = gpar(col = "black", fontsize = 8),
...
ch <- res_heatmap(fr, metrics_wide, 
                  filename = file.path(FIG_DIR, 'ranking_squair_et_al_combined_big.pdf'), 
                  fig_width = 8, fig_height = 48)
```
```{r}
ch <- res_heatmap(fr, metrics_wide, 
                  filename = file.path(FIG_DIR, 'ranking_squair_et_al_combined_big_squished.pdf'), 
                  fig_width = 8, fig_height = 3.5, show_values = FALSE, 
...
ch <- res_heatmap(fr, metrics_wide, idx = idx,
                  filename = file.path(FIG_DIR, 'ranking_squair_et_al_combined_filter1.pdf'), 
...
ch <- res_heatmap(fr, metrics_wide, idx = idx,
                  filename = file.path(FIG_DIR, 'ranking_squair_et_al_combined_filter2.pdf'), 
