...
```{r}
res_all_genes <- filter(sum_res, is.na(expr_grp))
ggplot(res_all_genes, aes(Precision)) +
  geom_histogram(binwidth = 0.02) +
  facet_wrap(~ comparison)

ggplot(res_all_genes, aes(Sensitivity)) +
  geom_histogram(binwidth = 0.02) +
  facet_wrap(~ comparison)

ggplot(res_all_genes, aes(GSS)) +
  geom_histogram(binwidth = 0.01) +
  facet_wrap(~ comparison)

ggplot(res_all_genes, aes(F1)) +
  geom_histogram(binwidth = 0.01) +
  facet_wrap(~ comparison)

ggplot(res_all_genes, aes(MCC)) +
  geom_histogram(binwidth = 0.01) +
  facet_wrap(~ comparison)
```

```{r}
ggplot(res_all_genes, aes(Sensitivity, Precision, color = comparison)) +
  geom_point()

ggplot(res_all_genes, aes(Sensitivity, Precision, color = cut(GSS, 11))) +
  geom_point() +
  facet_wrap(~ comparison) +
  theme_dark() +
  scale_color_viridis_d() 

ggplot(res_all_genes, aes(Sensitivity, Precision, color = cut(F1, 11))) +
  geom_point() +
  facet_wrap(~ comparison) +
  theme_dark() +
  scale_color_viridis_d() 

ggplot(res_all_genes, aes(Sensitivity, Precision, color = cut(MCC, 11))) +
  geom_point() +
  facet_wrap(~ comparison) +
  theme_dark() +
  scale_color_viridis_d() 
...
```{r}
res_all_genes <- group_by(res_all_genes, comparison) %>%
  mutate(Precision_rel = scales::rescale(Precision),
         ...
         MCC_rescaled = scales::rescale(scales::oob_squish_any(MCC)))
ggplot(res_all_genes, aes(Sensitivity_rel, Precision_rel, color = comparison)) +
  geom_point()
ggplot(res_all_genes, aes(Sensitivity, Precision, color = GSS_rescaled)) +
  geom_point() +
  facet_wrap(~ comparison, scales = 'free') +
  theme_dark() +
  scale_color_viridis_c() 
ggplot(res_all_genes, aes(Sensitivity, Precision, color = MCC_rescaled)) +
  geom_point() +
  facet_wrap(~ comparison, scales = 'free') +
  theme_dark() +
  scale_color_viridis_c() 
```
Per expression group

```{r}
res_grouped_genes <- filter(sum_res, !is.na(expr_grp))

ggplot(res_grouped_genes, aes(Precision)) +
  geom_histogram(binwidth = 0.02) +
  facet_grid(expr_grp ~ comparison)

ggplot(res_grouped_genes, aes(Sensitivity)) +
  geom_histogram(binwidth = 0.02) +
  facet_grid(expr_grp ~ comparison)

ggplot(res_grouped_genes, aes(GSS)) +
  geom_histogram(binwidth = 0.01) +
  facet_grid(expr_grp ~ comparison)

ggplot(res_grouped_genes, aes(F1)) +
  geom_histogram(binwidth = 0.01) +
  facet_grid(expr_grp ~ comparison)

ggplot(res_grouped_genes, aes(MCC)) +
  geom_histogram(binwidth = 0.01) +
  facet_grid(expr_grp ~ comparison)

ggplot(res_grouped_genes, aes(FDR)) +
  geom_histogram(binwidth = 0.01) +
  facet_grid(expr_grp ~ comparison)
```
...
GSS ranking
```{r}
fr <- group_by(res_all_genes, transformation, de_method, pipeline) %>%
...
fr <- res_all_genes %>%
  ...
  summarise(time = sum(time), GSS = mean(GSS_rescaled), .groups = 'drop') 

fdr_low <- sum_res %>%
  filter(!is.na(expr_grp), expr_grp == 'low') %>%
  group_by(comparison) %>%
  mutate(FDR_rel = scales::rescale(FDR)) %>%
  group_by(transformation, de_method, pipeline) %>%
  summarise(FDR = mean(FDR_rel), .groups = 'drop') %>%
  dplyr::rename(FDR_low = FDR)

fdr_high <- sum_res %>%
  filter(!is.na(expr_grp), expr_grp == 'high') %>%
  group_by(comparison) %>%
  mutate(FDR_rel = scales::rescale(FDR)) %>%
  group_by(transformation, de_method, pipeline) %>%
  summarise(FDR = mean(FDR_rel), .groups = 'drop') %>%
...
```{r}
metrics_wide <- arrange(res_all_genes, comp) %>%
                  tidyr::pivot_wider(
