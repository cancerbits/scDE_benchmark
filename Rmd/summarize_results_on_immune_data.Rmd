...
for (comp in comps) {
  ...
  genesets <- c(gobp_genesets, sigdb_genesets[ names(sigdb_genesets) %in% sigdb_expected_genesets[[comp]] ])
  ...
  gsea_list <- future_lapply(rds_files, FUN = function(f) {
    ...
    gsea_res <- de_res %>% 
      summarise(cb_hyper(signature = as.character(feature[pred_de]), 
  ...
  gsea_res <- as_tibble(data.table::rbindlist(gsea_list)) %>%
    ...
    mutate(is_gobp = grepl(pattern = '\\(GO:\\d+\\)$', x = label),
           ...
           is_sigdb_expected = label %in% sigdb_expected_genesets[[comp]])
