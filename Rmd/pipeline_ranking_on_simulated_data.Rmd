...
```{r}
library('ggplot2')
ggplot(res, aes(Precision, Sensitivity, color = cut(GSS, 11))) + 
  geom_point() + 
  scale_color_viridis_d() +
  facet_grid(expr_grp ~ experiment, scales = 'free')
```
```{r}
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(GSS_rescaled = scales::rescale(scales::oob_squish_any(GSS))) %>%
  ggplot(aes(Precision, Sensitivity, color = GSS_rescaled)) + 
  geom_point() + 
...
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(BA_rescaled = scales::rescale(scales::oob_squish_any(BA))) %>%
  ggplot(aes(Precision, Sensitivity, color = BA_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
```
...
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(F1_rescaled = scales::rescale(scales::oob_squish_any(F1))) %>%
  ggplot(aes(Precision, Sensitivity, color = F1_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
```
...
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  ggplot(aes(Precision, Sensitivity, color = MCC)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(MCC_rescaled = scales::rescale(scales::oob_squish_any(MCC))) %>%
  ggplot(aes(Precision, Sensitivity, color = MCC_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
filter(res, !is.na(expr_grp)) %>%
  group_by(experiment, expr_grp) %>%
  mutate(MCC_rescaled = scales::rescale(scales::oob_squish_any(MCC))) %>%
  ggplot(aes(Precision, Sensitivity, color = MCC_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment + expr_grp, scales = 'free', dir = 'v', ncol = 4)
```

```{r}
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(FM_rescaled = scales::rescale(scales::oob_squish_any(FM))) %>%
  ggplot(aes(Precision, Sensitivity, color = FM_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
```
...
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(CSI_rescaled = scales::rescale(scales::oob_squish_any(CSI))) %>%
  ggplot(aes(Precision, Sensitivity, color = CSI_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
```
...
filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(PSS_rescaled = scales::rescale(scales::oob_squish_any(PSS))) %>%
  ggplot(aes(Precision, Sensitivity, color = PSS_rescaled)) + 
  geom_point() + 
  scale_color_viridis_c() +
  facet_wrap( ~ experiment, scales = 'free')
```
...
fr <- filter(res, !is.na(expr_grp)) %>%
  group_by(experiment, expr_grp) %>%
  mutate(MCC_rescaled = scales::rescale(scales::oob_squish_any(MCC))) %>%
  group_by(transformation, de_method, pipeline, expr_grp) %>%
  summarise(n = n(), MCC = mean(MCC_rescaled), .groups = 'drop') %>%
  tidyr::pivot_wider(id_cols = c(transformation, de_method, pipeline), names_from = expr_grp, values_from = MCC) %>%
  group_by(transformation, de_method, pipeline) %>%
  mutate(score = mean(c(low, medium, high))) %>%
  arrange(-score)
head(fr, 33) %>% as.data.frame()
```
...
```{r}
# rescale the GSS per experiment
fr <- filter(res, is.na(expr_grp)) %>%
  group_by(experiment) %>%
  mutate(GSS_rescaled = scales::rescale(scales::oob_squish_any(GSS))) %>%
  group_by(transformation, de_method, pipeline) %>%
  summarise(n = n(), GSS = mean(GSS_rescaled), .groups = 'drop') 
