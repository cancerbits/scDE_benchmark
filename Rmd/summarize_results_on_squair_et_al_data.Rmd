...
res_sum <- future_lapply(res_files, function(f) {
  f_parts <- gsub(pattern = ds_name, replacement = '', x = basename(f)) %>%
  ...
  transformation_time = res$timing['transformation', 'user.self'] 
  method_time = res$timing['de_method', 'user.self'] 
    
  # calculate performance metrics
  performance <- summarise(de_res, perf_metrics(pred_de3, bulk_de3)) %>%
    mutate(time_transformation = transformation_time, time_de_method = method_time)
  ...
  performance_sum %>%
    mutate(comparison = comp, transformation = transformation, de_method = de_method) 
}) %>% bind_rows()
