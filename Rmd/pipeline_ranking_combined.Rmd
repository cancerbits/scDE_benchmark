...
res <- left_join(res_immune, res_sim, by = c('transformation', 'de_method', 'pipeline')) %>%
...
```{r}
p1 <- ggplot(res, aes(score.x, score.y, color = `Uses pseudo-bulk`)) +
  geom_rect(aes(xmin = min(res_top$score.x), xmax = max(res_top$score.x), 
                ymin = min(res_top$score.y), ymax = max(res_top$score.y)),
                color = 'gray88', fill = 'gray88') +
  geom_point() +
  xlab('Simulation score') +
  ylab('Immune data score') +
...
p1 <- filter(res, !`Uses pseudo-bulk`) %>%
  ggplot(aes(score.x, score.y, color = `Uses pseudo-bulk`)) +
  geom_rect(aes(xmin = min(res_top$score.x), xmax = max(res_top$score.x), 
                ymin = min(res_top$score.y), ymax = max(res_top$score.y)),
                color = 'gray88', fill = 'gray88') +
  geom_point() +
